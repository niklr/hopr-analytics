...
function handleAccountSnapshotUpdate(address: Address, value: BigDecimal, block: BigInt, timestamp: BigInt, isFrom: boolean, stats: StatsContainer): void {
  ...
  // handleAccountUpdate
  const newTotalBalance = isAdd ? account.balance.plus(value) : account.balance.minus(value);
  account.balance = newTotalBalance;
  snapshot.balance = newTotalBalance;
